...
def send_to_session(digest: str) -> None:
    """
    Save Genesis digest to file for Monday session.
    NO notifications - only file storage and GitHub.
    """
    # Write to trigger file that monday.py checks (use .tmp/ not /tmp/)
    trigger_file = REPO_ROOT / ".tmp" / "genesis_monday_message.txt"

    try:
        trigger_file.parent.mkdir(parents=True, exist_ok=True)
        with open(trigger_file, 'w', encoding='utf-8') as f:
            f.write(digest)
        logger.info("✓ Saved to Monday trigger file (no notification)")
    except Exception as e:
